...
namespace {
    inline std::pair<int,int> bshape(int r1,int c1,int r2,int c2){
        ...
        if (!row_ok || !col_ok){
          std::cerr << "\n[Broadcast Error] Incompatible shapes detected!\n";
        std::cerr << "  Left tensor  shape: (" << r1 << ", " << c1 << ")\n";
        std::cerr << "  Right tensor shape: (" << r2 << ", " << c2 << ")\n";
        std::cerr << "  Row compatibility: " << (row_ok ? "OK" : "FAIL") << "\n";
        std::cerr << "  Col compatibility: " << (col_ok ? "OK" : "FAIL") << "\n";  
...
Tensor& Tensor::add_(const Tensor& g) {
    std::cout << "[Debug] add_ called on Tensor of shape (" << r_ << ", " << c_ << ") on device " 
              << (is_cpu() ? "CPU" : "CUDA") << "\n";
...
Tensor Tensor::clone(const Tensor& x) {
    Tensor y(x.rows(), x.cols(), x.device());
