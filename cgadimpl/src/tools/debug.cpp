...
thread_local std::vector<ag::debug::NodeCreatedCb> tl_node_created_cbs;
...
void on_node_created(const std::shared_ptr<Node>& n){
    ...
    print_tensor(label.str(), n->value);
        ...
        // Invoke the active callback (top of stack) if present.
    if (!tl_node_created_cbs.empty()) {
        try {
            auto &cb = tl_node_created_cbs.back();
            if (cb) cb(n);
        } catch (...) {
            // swallow exceptions to avoid crashing the runtime on user callback errors.
        }
    }
}
void set_node_created_callback(NodeCreatedCb cb) {
    tl_node_created_cbs.push_back(std::move(cb));
}

void clear_node_created_callback() {
    if (!tl_node_created_cbs.empty()) {
        tl_node_created_cbs.pop_back();
    } else {
