...
// Identify all nodes that are direct inputs (parents) of checkpointed nodes
static std::unordered_set<Node*> gather_checkpoint_parents(const std::vector<Node*>& topo) {
    std::unordered_set<Node*> protected_parents;
    for (Node* n : topo) {
        if (!n || !n->is_checkpoint) continue;
        for (auto& p : n->inputs) {
            if (p)
                protected_parents.insert(p.get());
        }
    }
    return protected_parents;
}
...
void sweep_safe_nodes(const Value& root, DeletePolicy policy) {
    ...
    auto protected_parents = gather_checkpoint_parents(order);
    int freed = 0;
    for (Node* n : order) {
        if (protected_parents.count(n)) continue;
        if (try_delete_node(n, policy))
            ++freed;
    }
    std::cout << "[careful_delete] Protected parents of checkpoints:\n";
for (auto* p : protected_parents)
    std::cout << "  " << (p->debug_name ? p->debug_name : op_name(p->op)) << "\n";
